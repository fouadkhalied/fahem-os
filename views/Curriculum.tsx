...
type ActiveNode =
  | { type: 'grade'; data: GradeLevelNode }
  | { type: 'subject'; data: SubjectNode }
  | null;

// ─── Pure helper: deep-clone a tree branch to avoid mutating shared refs ───────
...
export const Curriculum: React.FC<CurriculumProps> = ({ language }) => {
  ...
  // ── Single source of truth for the tree (grades + subjects + local mutations) ─
  ...
  // Track which grade IDs have had their subjects loaded from the DB
  const loadedGradeIds = useRef<Set<string>>(new Set());
  ...
  // ── Sync tree skeleton when DB grades change (don't wipe existing subjects) ───
  useEffect(() => {
    if (!curriculum.selectedSystem) {
      setTree(null);
      loadedGradeIds.current.clear();
      return;
    }
    ...
    setTree(prev => {
      const grades = curriculum.gradeNodes.map(g => ({
        ...g,
        // Preserve any already-loaded / locally-mutated subjects
  ...
  // ── Lazy-load subjects for the active grade ────────────────────────────────
  // Stable function that doesn't close over stale `tree`
  const loadSubjectsForGrade = useCallback(async (gradeId: string) => {
    if (loadedGradeIds.current.has(gradeId)) return;
    loadedGradeIds.current.add(gradeId);
  ...
  // Derive the active grade ID to use as a stable effect dependency
  const activeGradeId =
    activeNode?.type === 'grade' ? activeNode.data.id : null;

  useEffect(() => {
    if (activeGradeId) {
      loadSubjectsForGrade(activeGradeId);
    }
  }, [activeGradeId, loadSubjectsForGrade]);

  // ── Derive live subject data from tree (fixes stale activeNode.data) ─────────
  const getLiveSubject = useCallback((subjectId: string): SubjectNode | undefined => {
    if (!tree) return undefined;
    for (const g of tree.grades) {
      const found = g.subjects.find(s => s.id === subjectId);
      if (found) return found;
    }
    return undefined;
  }, [tree]);

  const getLiveGrade = useCallback((gradeId: string): GradeLevelNode | undefined => {
    return tree?.grades.find(g => g.id === gradeId);
  }, [tree]);

  // ── Tree mutation helper ──────────────────────────────────────────────────────
  // All mutations go through this to avoid shallow-clone bugs
  const mutateTree = useCallback((
    updater: (grades: GradeLevelNode[]) => GradeLevelNode[]
  ) => {
    setTree(prev => {
      if (!prev) return prev;
      return { ...prev, grades: updater(cloneTree(prev.grades)) };
    });
  }, []);

  // ── Handlers ──────────────────────────────────────────────────────────────────

  const handleCreateSystem = useCallback(async (system: CurriculumSystem) => {
    await curriculum.createSystem(system);
  }, [curriculum]);

  const handleGenerateAIPlan = useCallback(async (subject: SubjectNode) => {
    if (!tree) return;
    ...
    try {
      ...
      if (plan && gradeNode) {
        ...
        // Re-fetch and patch the grade's subjects in tree
        const subs = await curriculum.getSubjects(gradeNode.id);
        mutateTree(grades =>
          grades.map(g => g.id === gradeNode.id ? { ...g, subjects: subs } : g)
        );
      }
    } catch (err) {
  ...
  }, [tree, curriculum, language, mutateTree]);

  const handleCreateFolder = useCallback(() => {
    if (!newFolderName.trim() || activeNode?.type !== 'subject') return;
    const subject = activeNode.data;
  ...
  }, [newFolderName, activeNode, currentFolderId, mutateTree]);

  const handleRenameFolder = useCallback((folderId: string) => {
    if (!newFolderName.trim() || activeNode?.type !== 'subject') return;
    const subjectId = activeNode.data.id;
  ...
  }, [newFolderName, activeNode, mutateTree]);

  const handleDeleteFolder = useCallback((folderId: string) => {
    if (activeNode?.type !== 'subject') return;
    const subjectId = activeNode.data.id;
  ...
  }, [activeNode, currentFolderId, mutateTree]);
        ...
        subjects: g.subjects.map(s => {
          ...
          if (currentFolderId) {
            return {
              ...
              folders: s.folders.map(f =>
                f.id === currentFolderId
                  ? { ...f, resources: f.resources.filter(r => r.id !== resourceId) }
                  : f
              )
            };
          }
  ...
  const handleFileUpload = useCallback((source: 'Local' | 'Google Drive' | 'OneDrive', fileName?: string) => {
    if (activeNode?.type !== 'subject') return;
    const subjectId = activeNode.data.id;
    const type = uploadType === 'Link' ? 'Document' : uploadType;
    const newResource: ContentResource = {
      id: `res-${Date.now()}`,
      title: fileName || `New ${type} Material`,
      type,
      url: '#',
      source,
        ...
        subjects: g.subjects.map(s => {
          ...
          if (currentFolderId) {
            return {
              ...
              folders: s.folders.map(f =>
                f.id === currentFolderId
                  ? { ...f, resources: [...f.resources, newResource] }
                  : f
              )
            };
          }
  ...
  }, [activeNode, uploadType, currentFolderId, mutateTree]);

  const handleAddSubject = useCallback(async () => {
    if (!newSubject.code || !newSubject.nameEn || !newSubject.nameAr || activeNode?.type !== 'grade') return;
    const grade = activeNode.data;
    ...
    try {
      ...
      // Force re-load for this grade on next visit
      loadedGradeIds.current.delete(grade.id);
      await loadSubjectsForGrade(grade.id);
    } catch (err) {
  ...
  }, [newSubject, activeNode, curriculum, loadSubjectsForGrade]);

  const handleExportSubjects = useCallback(() => {
    if (activeNode?.type !== 'grade' || !tree) return;
    const grade = getLiveGrade(activeNode.data.id);
    if (!grade) return;

    const headers = ['Code', 'NameEn', 'NameAr', 'Department'];
    const rows = grade.subjects.map(s => [
      s.code || '', s.nameEn || '', s.nameAr || '', s.department || ''
    ]);
  ...
  }, [activeNode, tree, getLiveGrade]);

  const downloadSubjectTemplate = useCallback(() => {
    const headers = ['Code', 'NameEn', 'NameAr', 'Department'];
  ...
  const handleImportSubjects = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    // Reset input so re-importing the same file triggers onChange again
    e.target.value = '';
    if (!file || activeNode?.type !== 'grade' || !tree) return;

    const gradeId = activeNode.data.id;
    ...
    reader.onload = (event) => {
      ...
      if (newSubjects.length > 0) {
        mutateTree(grades =>
          grades.map(g =>
            g.id === gradeId
              ? { ...g, subjects: [...g.subjects, ...newSubjects] }
              : g
          )
        );
      }
  ...
  }, [activeNode, tree, isRTL, mutateTree]);

  const handleToggleTeacher = useCallback(async (teacherId: string) => {
    if (!selectedSubjectForTeachers || !tree) return;

    const gradeNode = tree.grades.find(g =>
      g.subjects.some(s => s.id === selectedSubjectForTeachers.id)
    );
  ...
  }, [selectedSubjectForTeachers, tree, curriculum, mutateTree]);

  const onLocalFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) handleFileUpload('Local', file.name);
  }, [handleFileUpload]);
  ...
  const goBack = useCallback(() => {
    ...
    if (activeNode?.type === 'subject' && tree) {
      const parentGrade = tree.grades.find(g =>
        g.subjects.some(s => s.id === activeNode.data.id)
      );
      if (parentGrade) {
    ...
    setActiveNode(null);
  }, [currentFolderId, activeNode, tree]);
  ...
  // ── Derived live data for the active subject (never stale) ────────────────────
  const liveActiveSubject =
    activeNode?.type === 'subject'
      ? getLiveSubject(activeNode.data.id) ?? activeNode.data
      : null;

  const liveActiveGrade =
    activeNode?.type === 'grade'
      ? getLiveGrade(activeNode.data.id) ?? activeNode.data
      : null;
      ...
      <div className="max-w-5xl mx-auto py-12 animate-fadeIn text-center">
        <div className="mb-12">
          ...
          <p className="text-lg text-gray-500 max-w-xl mx-auto">
            Select an educational framework to automatically generate grade levels, subjects, and academic calendars.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {(['National', 'IG', 'IB', 'American'] as CurriculumSystem[]).map(sys => (
            ...
            >
              <div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-orange-400 to-orange-600 opacity-0 group-hover:opacity-100 transition-opacity" />
        ...
        {activeNode && (
          <div className="lg:hidden flex items-center gap-2 mb-2">
            ...
            <span className="font-bold text-gray-900">
              {activeNode.type === 'grade'
                ? (liveActiveGrade?.name ?? activeNode.data.name)
                : (liveActiveSubject?.name ?? activeNode.data.name)}
            </span>
          </div>
        )}
              ...
              <div key={grade.id} className="mb-2">
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded-xl cursor-pointer transition-colors group
                    ${activeNode?.data.id === grade.id ? 'bg-primary-50 text-primary-800' : 'hover:bg-gray-50'}`}
                >
                  <Folder
                    size={20}
                    className={activeNode?.data.id === grade.id
                      ? 'text-primary-600 fill-primary-200'
                      : 'text-gray-400 fill-gray-50 group-hover:text-primary-400 group-hover:fill-primary-100'}
                  />
                      ...
                      {grade.subjects.map(subject => (
                        <div
                          ...
                          className={`flex items-center gap-3 p-2 rounded-lg cursor-pointer transition-colors
                            ${activeNode?.type === 'subject' && activeNode.data.id === subject.id
        ...
        `}>
          {!activeNode ? (
            ...
            <div className="flex-1 overflow-y-auto p-4 lg:p-8">
              ...
              {/* ── GRADE VIEW ────────────────────────────────────────────────── */}
              {activeNode.type === 'grade' && liveActiveGrade && (
                <div>
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                      <h2 className="text-2xl lg:text-3xl font-extrabold text-gray-900">{liveActiveGrade.name}</h2>
                      <p className="text-sm text-gray-500 mt-1">{liveActiveGrade.subjects.length} Subjects Total</p>
                    </div>
                      ...
                      .map(sub => (
                        ...
                        >
                          <div className="flex justify-between items-start mb-4">
                            ...
                            <span className="bg-white text-gray-500 text-[10px] font-bold px-2 py-1 rounded border border-gray-100 uppercase">
                              {sub.weeks.length} Weeks
                            </span>
                          </div>
                          ...
                          <div className="flex gap-2">
                            ...
                            >
                              Teachers {sub.assignedTeacherIds?.length ? `(${sub.assignedTeacherIds.length})` : ''}
                            </Button>
                          </div>
                        </div>
                      ))}
              ...
              {/* ── SUBJECT VIEW ─────────────────────────────────────────────── */}
              {activeNode.type === 'subject' && liveActiveSubject && (
                <div className="space-y-8">
                  <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end gap-4 pb-6 border-b border-gray-100">
                    <div>
                      ...
                      <h2 className="text-2xl lg:text-3xl font-extrabold text-gray-900">{liveActiveSubject.name}</h2>
                    </div>
                    <Button
                      onClick={() => handleGenerateAIPlan(liveActiveSubject)}
                      isLoading={generatingLesson}
                      className="shadow-orange-200 w-full lg:w-auto"
                    >
                  ...
                  <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
                    ...
                    <div className="xl:col-span-2 space-y-6">
                      ...
                      <div className="border-2 border-dashed border-gray-200 rounded-2xl p-8 text-center bg-gray-50/50 hover:bg-orange-50/30 hover:border-orange-200 transition-colors group">
                        ...
                        <div className="flex flex-wrap justify-center gap-3">
                          {(['Document', 'Video', 'Presentation', 'SCORM'] as const).map(type => (
                            <button
                              key={type}
                              onClick={() => { setUploadType(type); setIsUploadModalOpen(true); }}
                      ...
                      <div>
                        <div className="flex justify-between items-center mb-4">
                          <h3 className="font-bold text-gray-900 flex items-center gap-2">
                            ...
                            <span className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full text-xs">
                              {currentFolderId
                                ? liveActiveSubject.folders.find(f => f.id === currentFolderId)?.resources.length ?? 0
                                : liveActiveSubject.resources.length + liveActiveSubject.folders.length}
                            </span>
                          </h3>
                          <div className="flex gap-2">
                            {currentFolderId && (
                        ...
                        <div className="space-y-3">
                          ...
                          {!currentFolderId && liveActiveSubject.folders.map(folder => (
                            <div
                              key={folder.id}
                          ...
                          {/* Resources */}
                          {(currentFolderId
                            ? liveActiveSubject.folders.find(f => f.id === currentFolderId)?.resources ?? []
                            : liveActiveSubject.resources
                          ).map(file => (
                            <div key={file.id} className="flex items-center justify-between p-4 rounded-2xl border border-gray-100 bg-white hover:shadow-md transition-all group">
                              <div className="flex items-center gap-4">
                                <div className={`w-10 h-10 rounded-lg flex items-center justify-center text-white font-bold text-xs
                                  ${file.type === 'Document' ? 'bg-red-500' : file.type === 'Presentation' ? 'bg-orange-500' : file.type === 'SCORM' ? 'bg-teal-500' : 'bg-blue-500'}`}>
                    ...
                    <div className="bg-gray-50 rounded-3xl p-6 h-fit border border-gray-100">
                      ...
                      <div className="relative border-l-2 border-gray-200 ml-3 space-y-8 pl-6 pb-2">
                        {liveActiveSubject.weeks.slice(0, 5).map(week => (
                          <div key={week.id} className="relative">
                            <span className="absolute -left-[31px] w-4 h-4 rounded-full bg-white border-2 border-orange-500" />
                        ...
                        <div className="relative">
                          <span className="absolute -left-[31px] w-4 h-4 rounded-full bg-gray-200" />
              ...
              <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 flex gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-blue-700">
                    {isRTL ? 'يجب أن يحتوي الملف على الأعمدة التالية:' : 'File must contain the following columns:'}<br />
